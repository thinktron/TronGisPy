...
def project_npidxs_to_XYZs(P_npidxs, P_Z, aerotri_params):
    """
    ...
    ks = mt_xyzs[2] / (L_Z - P_Z) # convert from (L_Z - P_Z) = P_xyzs[2] / k, and query for k
    P_XYZs = (L_XYZ - (mt_xyzs / ks).T) # 4 points, 3 dim
    return P_XYZs

def project_XYZs_to_npidxs(P_XYZs, aerotri_params):
    """
